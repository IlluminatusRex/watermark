...
const startApp = async () => {
  ...
  if (options.watermarkType === 'Text watermark') {
    if (fs.existsSync(inputImagePath)) {
      ...
      await addTextWatermarkToImage(inputImagePath, outputImagePath, options.watermarkText);
    ...
    if (fs.existsSync(inputImagePath) && fs.existsSync(watermarkImagePath)) {
      const outputImagePath = './img/' + prepareOutputFilename(options.inputImage);
      await addImageWatermarkToImage(inputImagePath, outputImagePath, watermarkImagePath);
      console.log('Task completed successfully');
      startApp();
    } else {
      console.log('Something went wrong... Try again');
